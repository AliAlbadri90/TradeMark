...
export class ViewLogComponent implements OnInit {
  ...
  loadPage(page?: number, dontNavigate?: boolean): void {
    ...
    this.viewLogService
      ...
      .subscribe({
        next: (res: HttpResponse<IViewLog[]>) => {
  ...
  trackId(index: number, item: IViewLog): number {
    return item.id!;
  }

  delete(viewLog: IViewLog): void {
    const modalRef = this.modalService.open(ViewLogDeleteDialogComponent, { size: 'lg', backdrop: 'static' });
    modalRef.componentInstance.viewLog = viewLog;
  ...
  protected onSuccess(data: IViewLog[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {

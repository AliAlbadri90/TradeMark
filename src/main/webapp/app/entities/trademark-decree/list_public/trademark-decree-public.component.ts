...
@Component({
  selector: 'jhi-trademark-decree-public',
  templateUrl: './trademark-decree-public.component.html',
})
export class TrademarkDecreePublicComponent implements OnInit {
  ...
  searchPublic(currentSearch: any): void {
  ...
  protected onSuccess(data: ITrademarkDecree[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
    ...
    this.page = page;
    if (navigate) {
      this.router.navigate(['/trademark-decree/public'], {
        queryParams: {
          'decreeNo.contains': this.currentSearch,
          'tradeMarkOwner.contains': this.currentSearch,
          'country.contains': this.currentSearch,

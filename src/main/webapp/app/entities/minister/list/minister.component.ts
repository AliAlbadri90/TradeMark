...
export class MinisterComponent implements OnInit {
  ...
  loadPage(page?: number, dontNavigate?: boolean): void {
    ...
    this.ministerService
      ...
      .subscribe({
        next: (res: HttpResponse<IMinister[]>) => {
  ...
  trackId(index: number, item: IMinister): number {
    return item.id!;
  }

  delete(minister: IMinister): void {
    const modalRef = this.modalService.open(MinisterDeleteDialogComponent, { size: 'lg', backdrop: 'static' });
  ...
  protected onSuccess(data: IMinister[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {

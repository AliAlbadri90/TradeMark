...
export class GovernmentComponent implements OnInit {
  ...
  loadPage(page?: number, dontNavigate?: boolean): void {
    ...
    this.governmentService
      ...
      .subscribe({
        next: (res: HttpResponse<IGovernment[]>) => {
  ...
  trackId(index: number, item: IGovernment): number {
    return item.id!;
  }

  delete(government: IGovernment): void {
    const modalRef = this.modalService.open(GovernmentDeleteDialogComponent, { size: 'lg', backdrop: 'static' });
    modalRef.componentInstance.government = government;
  ...
  protected onSuccess(data: IGovernment[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
